...
for key, value in dev.items():
    plt.figure(f'Device_{key}')
    color_count = 0
    print('key',key)
    num_colors = len(value)
    colors = plt.cm.jet(np.linspace(0,1,num_colors))
    for r in value:
        print('color_count',color_count)
        print('r',r.filename)
        #r.network.plot_s_mag(m=1, n=0, color=colors[color_count], linestyle='dashed',label = f'Raw: {r.label}')  # Plot only s21 with colorblind colormap
        OS.deembed(r.network).plot_s_mag(m=1, n=0, color=colors[color_count], label = f'OS: {r.filename}')  # Plot only s21 with colorblind colormap          
        color_count += 1  
